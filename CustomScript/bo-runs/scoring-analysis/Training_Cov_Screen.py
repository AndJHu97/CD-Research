...
def parse_frankenstein_warheads(raw) -> set[str]:
    ...
    A single warhead name may contain commas; treat the full field as one name
    when it does not look like a multi-warhead list (comma + space separators
    between known-style entries). Prefer exact full-string inclusion always.
    """
    if pd.isna(raw):
        return set()
    text = str(raw).strip()
    ...
    # Always keep the unsplit name so comma-containing warheads are preserved.
    if full:
        parts.add(full)
    return parts
