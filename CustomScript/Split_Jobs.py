...
def split_csv(input_csv: Path, lines_per_file: int = 10) -> list[Path]:
	"""
	...
	for i, start in enumerate(range(0, len(rows), data_rows_per_file), start=1):
...
def main() -> None:
	...
	output_files = split_csv(input_csv, lines_per_file=args.lines_per_file)
